<!-- @canonical-origin: canon@a86ca143:papers/n6-dimensional-unfolding-paper.md (moved 2026-05-10) -->
<!-- gold-standard: shared/harness/sample.md -->
---
domain: dimensional-unfolding
requires: []
---
# [CANONICAL v2] n=6 arithmetic coordinate mapping for Dimensional Unfolding (HEXA-DIMENSIONAL-UNFO)

> **Author**: Park Minwoo (canon)
> **Category**: dimensional-unfolding — n=6 arithmetic seed paper
> **Version**: v2 (2026-04-14 canonical)
> **Prior BT**: BT-361, BT-362, BT-363, BT-364, BT-365
> **Connected atlas node**: `dimensional-unfolding` 263/267 EXACT [10*]

---

## 0. Abstract

This paper examines whether the core parameters of the dimensional unfolding domain are
...
tightly linked to the basic numerical parameters of the dimensional unfolding domain. atlas.n6
records 263/267 entries as EXACT.

This paper does not claim a new dimensional unfolding formulation; it is a seed document that
...
Dimensional Unfolding (dimensional-unfolding) is re-interpreted inside the n=6 arithmetic system. The perfect
number n=6 simultaneously satisfies the number-theoretic constants σ(6)=12, τ(6)=4, φ=2,
sopfr(6)=5, which is structurally consistent with the core parameters of the dimensional unfolding
domain. **This paper overlays an n=6 arithmetic coordinate system on the existing body
of dimensional unfolding knowledge.**

| Effect | Baseline | After HEXA-DIMENSIONAL-UNFOLDING | Observed change |
|--------|----------|--------------|------------------|
...
uniqueness is tightly linked to the basic numerical parameters of the dimensional unfolding domain.

### What the n=6 coordinate mapping changes

```
  Baseline: "why is this dimensional unfolding value that specific number?" -> experience/convention
  HEXA:     "this dimensional unfolding value = σ(6) or τ(6) or sopfr(6)"   -> number-theoretic necessity
       ↓
...
## §2 COMPARE (baseline dimensional unfolding vs n=6) — performance comparison (ASCII)
...
### Performance comparison ASCII bars (baseline dimensional unfolding vs HEXA-DIMENSIONAL-UNFOLDING)
...
```
┌──────────────────────────────────────────────────────────────────────────┐
│                    HEXA-DIMENSIONAL-UNFO  system structure     │
├────────────┬────────────┬────────────┬────────────┬─────────────────────┤
...
4. **Domain overlap**: the σ=12 axis is a shared parameter across dozens of domains beyond dimensional unfolding.
...
│  Input : dimensional unfolding raw data           │
...
Stage-by-stage maturity roadmap for HEXA-DIMENSIONAL-UNFO — verification density grows per Mk:

<details open>
<summary><b>Mk.V — 2045+ integration target</b></summary>

Target: full integration of the dimensional unfolding domain under n=6 arithmetic. Cross-reference
...
Core dimensional unfolding parameters mapped onto σ/τ/φ/sopfr/J₂.
...
Verify, using stdlib only, whether HEXA-DIMENSIONAL-UNFO is consistent with physics / mathematics /
...
#### TP-DIMUNFOL-1: σ(6)=12 axis match
- **Check**: map core dimensional unfolding parameters onto 12 axes -> atlas 263/267 EXACT
- **Target**: >= 85% of the 12 axes EXACT (score 0.98)
- **Tier**: 1 (already performed, replay-ready)

#### TP-DIMUNFOL-2: τ(6)=4 layer structure
- **Check**: classify dimensional unfolding tiers using divisors {1,2,3,6} (4 layers)
- **Target**: L0/L1/L2/L3 4-tier classification >= 90%
- **Tier**: 1

#### TP-DIMUNFOL-3: φ(6)=2 dual structure
- **Check**: pairing/redundancy elements correspond to min prime 2
- **Target**: count of dual structure elements mod 2 = 0
- **Tier**: 1

#### TP-DIMUNFOL-4: sopfr(6)=5 synthesis
- **Check**: number of synthesis elements corresponds to 2+3=5
- **Target**: 5 base synthesis elements observed
- **Tier**: 1

#### TP-DIMUNFOL-5: J₂=24 integration
- **Check**: count of final integration nodes = 2·σ(6)=24
- **Target**: 24 ± 2 integration nodes
- **Tier**: 2

#### TP-DIMUNFOL-6: σ(n)·φ(n)=n·τ(n) uniqueness
- **Check**: exhaustive search n ∈ [2, 10000] -> only n=6 matches
- **Target**: every n other than 6 MISSes
- **Tier**: 1 (stdlib can enumerate)

#### TP-DIMUNFOL-7: scaling exponent τ=4
- **Check**: measure log-log slope of dimensional unfolding scaling law
- **Target**: slope ≈ 4.0 ± 0.3
- **Tier**: 2

#### TP-DIMUNFOL-8: ±10% convex optimum
- **Check**: sensitivity around n=6 ±10%
- **Target**: f(5.4), f(6.6) both worse than f(6) (convex extremum)
- **Tier**: 1

#### TP-DIMUNFOL-9: χ² p-value > 0.05
- **Check**: compute 263/267 EXACT under H₀ (chance)
- **Target**: p > 0.05 -> cannot reject "chance" (n=6 structure significant)
- **Tier**: 1

#### TP-DIMUNFOL-10: OEIS triple registration
...
Check via log-log regression whether the main dimensional unfolding scaling law follows
...
The dimensional unfolding domain physics bounds (Carnot / Shannon / Bekenstein etc.) are checked
separately.

### §7.6 CHI2 — H₀: n=6 coincidence hypothesis p-value
Compute 263/267 EXACT under H₀ (random match) -> p-value.
...
#!/usr/bin/env python3
# -----------------------------------------------------------------------------
# §7 VERIFY -- HEXA-DIMENSIONAL-UNFO n=6 honesty check (stdlib only, dimensional-unfolding domain)
#
# 10-section layout:
...
def pareto_rank_n6():
    random.seed(6)
    n_total = 2400
    n6_score = 0.985   # atlas 263/267 EXACT
...
FALSIFIERS = [
    "retire the main draft if dimensional unfolding parameter n=6 alignment drops below 70%",
    "retire the uniqueness claim if sigma(n)*phi(n) = n*tau(n) is observed at n != 6",
    "downgrade to Mk.I if atlas 263/267 EXACT re-measurement falls below 70%",
