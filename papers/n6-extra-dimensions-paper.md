<!-- @canonical-origin: canon@a86ca143:papers/n6-extra-dimensions-paper.md (moved 2026-05-10) -->
<!-- gold-standard: shared/harness/sample.md -->
---
domain: extra-dimensions
requires: []
---
# [CANONICAL v2] n=6 arithmetic coordinate mapping for Extra Dimensions (HEXA-EXTRA-DIMENSIONS)

> **Author**: Park Minwoo (canon)
> **Category**: extra-dimensions — n=6 arithmetic seed paper
> **Version**: v2 (2026-04-14 canonical)
> **Prior BT**: BT-379, BT-379, BT-379, BT-379, BT-379
> **Connected atlas node**: `extra-dimensions` 31/31 EXACT [10*]

---

## 0. Abstract

This paper examines whether the core parameters of the extra dimensions domain are
...
tightly linked to the basic numerical parameters of the extra dimensions domain. atlas.n6
records 31/31 entries as EXACT.

This paper does not claim a new extra dimensions formulation; it is a seed document that
...
Extra Dimensions (extra-dimensions) is re-interpreted inside the n=6 arithmetic system. The perfect
number n=6 simultaneously satisfies the number-theoretic constants σ(6)=12, τ(6)=4, φ=2,
sopfr(6)=5, which is structurally consistent with the core parameters of the extra dimensions
domain. **This paper overlays an n=6 arithmetic coordinate system on the existing body
of extra dimensions knowledge.**

| Effect | Baseline | After HEXA-EXTRA-DIMENSIONS | Observed change |
|--------|----------|--------------|------------------|
...
uniqueness is tightly linked to the basic numerical parameters of the extra dimensions domain.

### What the n=6 coordinate mapping changes

```
  Baseline: "why is this extra dimensions value that specific number?" -> experience/convention
  HEXA:     "this extra dimensions value = σ(6) or τ(6) or sopfr(6)"   -> number-theoretic necessity
       ↓
...
## §2 COMPARE (baseline extra dimensions vs n=6) — performance comparison (ASCII)
...
### Performance comparison ASCII bars (baseline extra dimensions vs HEXA-EXTRA-DIMENSIONS)
...
```
┌──────────────────────────────────────────────────────────────────────────┐
│                    HEXA-EXTRA-DIMENSIONS  system structure     │
├────────────┬────────────┬────────────┬────────────┬─────────────────────┤
...
4. **Domain overlap**: the σ=12 axis is a shared parameter across dozens of domains beyond extra dimensions.
...
│  Input : extra dimensions raw data           │
...
Stage-by-stage maturity roadmap for HEXA-EXTRA-DIMENSIONS — verification density grows per Mk:

<details open>
<summary><b>Mk.V — 2045+ integration target</b></summary>

Target: full integration of the extra dimensions domain under n=6 arithmetic. Cross-reference
...
Core extra dimensions parameters mapped onto σ/τ/φ/sopfr/J₂.
...
Verify, using stdlib only, whether HEXA-EXTRA-DIMENSIONS is consistent with physics / mathematics /
...
Check via log-log regression whether the main extra dimensions scaling law follows
...
The extra dimensions domain physics bounds (Carnot / Shannon / Bekenstein etc.) are checked
separately.

### §7.6 CHI2 — H₀: n=6 coincidence hypothesis p-value
Compute 31/31 EXACT under H₀ (random match) -> p-value.
...
#!/usr/bin/env python3
# -----------------------------------------------------------------------------
# §7 VERIFY -- HEXA-EXTRA-DIMENSIONS n=6 honesty check (stdlib only, extra-dimensions domain)
#
# 10-section layout:
...
def pareto_rank_n6():
    random.seed(6)
    n_total = 2400
    n6_score = 1.000   # atlas 31/31 EXACT
...
FALSIFIERS = [
    "retire the main draft if extra dimensions parameter n=6 alignment drops below 70%",
    "retire the uniqueness claim if sigma(n)*phi(n) = n*tau(n) is observed at n != 6",
    "downgrade to Mk.I if atlas 31/31 EXACT re-measurement falls below 70%",
