<!-- @canonical-origin: canon@a86ca143:papers/n6-curvature-geometry-paper.md (moved 2026-05-10) -->
<!-- gold-standard: shared/harness/sample.md -->
---
domain: curvature-geometry
requires: []
---
# [CANONICAL v2] n=6 arithmetic coordinate mapping for Curvature & Geometry (HEXA-CURVATURE-GEOMET)

> **Author**: Park Minwoo (canon)
> **Category**: curvature-geometry — n=6 arithmetic seed paper
> **Version**: v2 (2026-04-14 canonical)
> **Prior BT**: BT-377, BT-377, BT-377, BT-377, BT-377
> **Connected atlas node**: `curvature-geometry` 35/35 EXACT [10*]

---

## 0. Abstract

This paper examines whether the core parameters of the curvature & geometry domain are
...
tightly linked to the basic numerical parameters of the curvature & geometry domain. atlas.n6
records 35/35 entries as EXACT.

This paper does not claim a new curvature & geometry formulation; it is a seed document that
...
Curvature & Geometry (curvature-geometry) is re-interpreted inside the n=6 arithmetic system. The perfect
number n=6 simultaneously satisfies the number-theoretic constants σ(6)=12, τ(6)=4, φ=2,
sopfr(6)=5, which is structurally consistent with the core parameters of the curvature & geometry
domain. **This paper overlays an n=6 arithmetic coordinate system on the existing body
of curvature & geometry knowledge.**

| Effect | Baseline | After HEXA-CURVATURE-GEOMETRY | Observed change |
|--------|----------|--------------|------------------|
...
uniqueness is tightly linked to the basic numerical parameters of the curvature & geometry domain.

### What the n=6 coordinate mapping changes

```
  Baseline: "why is this curvature & geometry value that specific number?" -> experience/convention
  HEXA:     "this curvature & geometry value = σ(6) or τ(6) or sopfr(6)"   -> number-theoretic necessity
       ↓
...
## §2 COMPARE (baseline curvature & geometry vs n=6) — performance comparison (ASCII)
...
### Performance comparison ASCII bars (baseline curvature & geometry vs HEXA-CURVATURE-GEOMETRY)
...
```
┌──────────────────────────────────────────────────────────────────────────┐
│                    HEXA-CURVATURE-GEOMET  system structure     │
├────────────┬────────────┬────────────┬────────────┬─────────────────────┤
...
4. **Domain overlap**: the σ=12 axis is a shared parameter across dozens of domains beyond curvature & geometry.
...
│  Input : curvature & geometry raw data           │
...
Stage-by-stage maturity roadmap for HEXA-CURVATURE-GEOMET — verification density grows per Mk:

<details open>
<summary><b>Mk.V — 2045+ integration target</b></summary>

Target: full integration of the curvature & geometry domain under n=6 arithmetic. Cross-reference
...
Core curvature & geometry parameters mapped onto σ/τ/φ/sopfr/J₂.
...
Verify, using stdlib only, whether HEXA-CURVATURE-GEOMET is consistent with physics / mathematics /
...
Check via log-log regression whether the main curvature & geometry scaling law follows
...
The curvature & geometry domain physics bounds (Carnot / Shannon / Bekenstein etc.) are checked
separately.

### §7.6 CHI2 — H₀: n=6 coincidence hypothesis p-value
Compute 35/35 EXACT under H₀ (random match) -> p-value.
...
#!/usr/bin/env python3
# -----------------------------------------------------------------------------
# §7 VERIFY -- HEXA-CURVATURE-GEOMET n=6 honesty check (stdlib only, curvature-geometry domain)
#
# 10-section layout:
...
FALSIFIERS = [
    "retire the main draft if curvature & geometry parameter n=6 alignment drops below 70%",
    "retire the uniqueness claim if sigma(n)*phi(n) = n*tau(n) is observed at n != 6",
    "downgrade to Mk.I if atlas 35/35 EXACT re-measurement falls below 70%",
